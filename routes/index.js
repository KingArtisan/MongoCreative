...
router.get('/commentName', function(req, res, next) {
console.log("In the GET route?");
var nam = req.param("name");
Comment.find({Name: nam}, function(err,commentList) { //Calls the find() method on your database
  if (err) return console.error(err); //If there's an error, print it out
  else {
    console.log(commentList); //Otherwise console log the comments you found
    res.json(commentList); //Then send the comments
...
router.get('/deleteComments', function(req, res, next){
